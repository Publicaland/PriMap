...
customElements.define('passkey-submit', class extends HTMLElement {
    ...
    async obtainCredential(useConditionalMediation, signal) {
        if (!browserSupportsPasskeys) {
            throw new Error('Unele funcții ale passkey-urilor lipsesc. Vă rugăm să vă actualizați browserul.');
        }
        ...
        if (this.attrs.operation === 'Create') {
            ...
            throw new Error(`Operațiunea passkey necunoscută '${this.attrs.operation}'.`);
        }
    }
    ...
    async obtainAndSubmitCredential(useConditionalMediation = false) {
        ...
        try {
            ...
            const errorMessage = error.name === 'NotAllowedError'
                ? 'Nu a fost furnizat niciun passkey de către autentificator.'
                : error.message;
